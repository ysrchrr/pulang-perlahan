...
@section('contents')
    <div class="row mb-3">
        <div class="col-md-6">
            <a href="{{ route('eval-narasumber') }}" class="btn btn-danger btn-sm">
...
@push('scripts')
    <script>
        ...
        function storeSoal() {
            $.ajax({
                type: "POST",
                url: "{{ route('eval-narasumber-soal-store') }}",
        ...
        function toEditSoal(id) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('eval-narasumber-soal-detail') }}",
                data: {
                    id: id
                },
                dataType: "json",
        ...
        function toRemoveSoal(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('eval-narasumber-soal-delete') }}",
                        type: 'POST',
                        data: {
                            id: id
                        },
        ...
        function loadTable() {
            ...
            $('#table-soal').DataTable({
                ...
                scrollX: true,
                ordering: false,
                ajax: "{{ route('eval-narasumber-soal-get-data', ['id_template' => enc_id($template->id)]) }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
